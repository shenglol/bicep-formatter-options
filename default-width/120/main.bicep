...
var evenDoggos = map(filter(numbers, i => contains(filter(numbers, j => 0 == j % 2), i)), x => doggos[x])
...
func buildUrl(https bool, host string, path string) string => '${https ? 'https' : 'http'}://${host}${empty(path)}'
